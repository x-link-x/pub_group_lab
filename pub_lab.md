...
Advanced extensions:
  ...
  - Add inheritance to your program: Create a new class of `Employee` that has `wage`, `wallet` and `age` attributes. `Employee` and `Customer` should inherit from `Person`. Add a list of `Employees` to the `Pub` and add a `pay_employees` method to the `Pub`. 
